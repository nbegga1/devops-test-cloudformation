pipeline{
    ...
    environment{
        ...
        STACK_CREATE = sh(script: "aws cloudformation describe-stacks --stack-name $STACK_NAME --region $AWS_REGION && stack_update=true || stack_create=true", returnStdout: true).trim()
        STACK_UPDATE = false
    }
